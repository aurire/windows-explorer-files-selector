...
namespace windowsExplorerFileSelector
{
	...
	class Program
	{
		...
		{
		    // args = new string[] {@"D:\linked\tst.txt"}; // for testing
			if (args.Length == 0) {
				Console.WriteLine("File selector by Aurimas Rekstys. All the real wor done by ");
            	System.Console.WriteLine("Please enter text file path. File should contain a list of files to be selected.");
	            System.Console.WriteLine("Usage: selector <filename>");
            	return;
        	}
			foreach (string arg in args)
			{
				if (File.Exists(arg))
		        {
			    	string foldName = Path.GetDirectoryName(arg);
			    	string[] readText = File.ReadAllLines(arg);
			    	List<string> validList = new List<string>();
			    	//if files does not have full path, add current directory to path
			    	for(int i = 0 ; i < readText.Length ; i++)
					{
			    		int index = readText[i].IndexOf("\\", StringComparison.CurrentCulture);
			    		if (index == -1) {
			    		    readText[i] = foldName + "\\" + readText[i];
			    		}
			    		if (File.Exists(readText[i])) {
                            validList.Add(readText[i]);
			    		}
					}
			    	ShowSelectedInExplorer.FilesOrFolders(validList.ToArray());
		        }
			}
		}
	}
